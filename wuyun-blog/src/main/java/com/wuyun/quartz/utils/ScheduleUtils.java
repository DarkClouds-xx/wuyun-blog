...
import com.wuyun.quartz.execution.QuartzDisallowConcurrentExecution;
import com.wuyun.quartz.execution.QuartzJobExecution;
import org.quartz.*;
    ...
    public static void createScheduleJob(Scheduler scheduler, Task task) {
        try {
            ...
            JobDetail jobDetail = JobBuilder.newJob(jobClass)
                    .withIdentity(getJobKey(taskId, taskGroup))
                    .build();
            // 表达式调度构建器
            ...
            // 触发器Trigger最基本的功能是指定任务的执行时间，执行间隔，运行次数等
            CronTrigger trigger = TriggerBuilder.newTrigger()
                    .withIdentity(getTriggerKey(taskId, taskGroup))
