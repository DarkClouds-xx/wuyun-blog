...
import com.wuyun.entity.*;
import com.wuyun.mapper.ArticleMapper;
import com.wuyun.mapper.CommentMapper;
...
import static com.wuyun.constant.RedisConstant.COMMENT_LIKE_COUNT;
...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public void updateCommentCheck(CheckDTO check) {
        ...
        List<Comment> commentList = check.getIdList().stream()
                ...
                .id(id).isCheck(check.getIsCheck())
                .build()
    ...
    public PageResult<CommentVO> listCommentVO(ConditionDTO condition) {
        ...
        // 父评论id集合
        List<Integer> parentCommentIdList = commentVOList.stream()
                .map(CommentVO::getId)
                .collect(Collectors.toList());
        // 分组查询每组父评论下的子评论前三条
        List<ReplyVO> replyVOList = commentMapper.selectReplyByParentIdList(parentCommentIdList);
        // 封装子评论点赞量
        replyVOList.forEach(item -> item.setLikeCount(Optional.ofNullable(likeCountMap
    ...
    private void verifyComment(CommentDTO comment) {
        ...
        // 如果父评论不为空，则评论为子评论，判断回复的评论和用户是否存在
